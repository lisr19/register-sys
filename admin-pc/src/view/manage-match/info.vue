	...
	<div>
		...
		<Modal v-model="isAdd" :mask-closable="false" :closable="false" title="添加：" width="600">
			<Form :model="addForm" ref="addForm" :rules="addRules" :label-width="130">
				<Card>
					<FormItem label="所属赛事：" prop="match_id">
					...
					<FormItem label="赛场：" prop="area">
						<Input clearable v-model="addForm.area" placeholder="请填写赛场"/>
					</FormItem>
					<FormItem label="比赛时间：" prop="match_time" >
						<DatePicker  type="datetimerange"  placeholder="请选择比赛时间"   v-model="addForm.match_time"
						            @on-change="v=>{addForm.match_time=v}"
						></DatePicker>
					</FormItem>
					<FormItem label="报名时间：" prop="end_time" >
						<DatePicker  type="datetimerange"  placeholder="请选择报名时间"   v-model="addForm.end_time"
						            @on-change="v=>{addForm.end_time=v}"
						></DatePicker>
					</FormItem>
				</Card>
			</Form>
		...
		<Modal v-model="isEdit" :mask-closable="false" :closable="false" title="编辑：" width="600">
			<Form :model="editForm" ref="editForm" :rules="editRules" :label-width="130">
				<Card>
					<FormItem label="所属赛事：" prop="match_name">
						<Input clearable disabled v-model="editForm.match_name" placeholder="请填写赛事名称"/>
					</FormItem>
					<FormItem label="赛场：" prop="area">
						<Input clearable v-model="editForm.area" placeholder="请填写赛场"/>
					</FormItem>
					<FormItem label="比赛时间：" prop="match_time" >
						<DatePicker  type="datetimerange"  placeholder="请选择比赛时间"   v-model="editForm.match_time"
						            @on-change="v=>{editForm.match_time=v}"
						></DatePicker>
					</FormItem>
					<FormItem label="报名时间：" prop="end_time" >
						<DatePicker  type="datetimerange"  placeholder="请选择报名时间"   v-model="editForm.end_time"
						            @on-change="v=>{editForm.end_time=v}"
						></DatePicker>
					</FormItem>
				</Card>
			</Form>
...
<script>
	import {getList,addList,delList,editList,getMatchList} from '@/api/match/info'
	export default {
		components: {
		...
		data () {
			return {
								...
								h('Button', {
									...
									on: {
										click: () => {
											...
											this.openEditModal(data, data.match_time, data.end_time)
										}
									}
								}, '编辑'),
								// h('Button', {
				...
				searchOption: {}, // 查询用参数
				addRules: {
					match_id: [ { required: true, message: '请选择所属赛事' }],
					area: [ { required: true, message: '请填写赛场' }],
					// match_time: { required: true, type: 'date', message: '请选择比赛时间', trigger: 'change' },
					// end_time: { required: true, type: 'date', message: '请选择报名时间', trigger: 'change' }
				},
				editRules: {
					match_name: [ { required: true, message: '赛事名称不能为空', trigger: 'blur' }],
					area: [ { required: true, message: '请填写赛场', trigger: 'blur' }],
				}
			}
		},
		created () {
		...
		methods: {
			...
			// 编辑modal打开
			openEditModal (params, time1, time2) {
				this.editForm = params
				this.editForm.match_time = []
				this.editForm.end_time = []
				this.editForm.match_time.push(time1.substr(0, 18))
				this.editForm.match_time.push(time1.substr(20, 18))
				this.editForm.end_time.push(time2.substr(0, 18))
				this.editForm.end_time.push(time2.substr(20, 18))
				setTimeout(()=>{
					this.isEdit = true
				},300)
			},
