<template>
  <div>
	  <!--<div>{{currentIndex}}</div>-->
    <div class="demo-upload-list" v-for="(item,index) in uploadList">
      <template v-if="item.status === 'finished'">
        <img :src="item.url">
        <div class="demo-upload-list-cover">
          <Icon type="ios-eye-outline" @click.native="handleView(item)"></Icon>
          <Icon type="ios-trash-outline" @click.native="handleRemove(item,index)"></Icon>
        </div>
      </template>
      <template v-else>
        <Progress v-if="item.showProgress" :percent="item.percentage" hide-info></Progress>
      </template>
    </div>
    <Upload
      v-show="uploadList.length<2"
      ref="upload"
      :show-upload-list="false"
      :on-success="handleSuccess"
      :format="['jpg','jpeg','png']"
      :max-size="10240"
      :on-format-error="handleFormatError"
      :on-exceeded-size="handleMaxSize"
      :before-upload="handleBeforeUpload"
      :headers="myHeader"
      type="drag"
      name="file"
      :action="baseUrl"
      style="display: inline-block;width:58px;">
      <div  style="width: 58px;height:58px;line-height: 58px;">
        <Icon type="ios-camera" size="20"></Icon>
      </div>
    </Upload>
    <Modal title="上传的图片" v-model="visible">
      <img :src="imgUrl" v-if="visible" style="width: 100%">
    </Modal>
  </div>
</template>
<script>
  export default {
    name: 'img-upload-more',
	  props:['currentIndex'],
    data () {
      return {
        myHeader: { authorization: localStorage.getItem('token') },
        imgName: '',
        imgUrl:'',
        visible: false,
        uploadList: [],
        baseUrl:'',
      }
    },
    methods: {
      handleView (file) {
        this.imgUrl = file.url
        this.visible = true
      },
      handleRemove (file,index) {
        this.$emit('delImg',index,this.currentIndex)
        const fileList = this.$refs.upload.fileList
        this.$refs.upload.fileList.splice(fileList.indexOf(file), 1)
      },
      handleSuccess (res, file) {
        this.$emit('imgUpload', res.url,this.currentIndex)
        file.url = res.url
        file.name = '图片'
      },
      // handleSuccess1 (res, file) {
      //   this.$emit('imgUpload1', res.url,this.currentIndex,index)
      //   file.url = res.url
      //   file.name = '图片'
      // },
      handleFormatError (file) {
        this.$Notice.warning({
          title: '上传的图片格式错误',
          desc: '上传的图片格式不对，只支持jpg,jpeg,png'
        })
      },
      handleMaxSize (file) {
        this.$Notice.warning({
          title: '超出上传限制大小',
          desc: '上传的图片最大不能超过10MB'
        })
      },
      handleBeforeUpload () {
	      this.$emit('imgUploadBefore')
        const check = this.uploadList.length < 2
        if (!check) {
          this.$Notice.warning({
            title: '最多只能上传两张.'
          })
        }
        return check
      },
      // 更新图片信息
      updateImgUrl (params) {
        this.$refs.upload.fileList = params
        this.uploadList = this.$refs.upload.fileList
      }
    },
    mounted () {
      this.uploadList = this.$refs.upload.fileList
    },
    created(){
      this.baseUrl = this.$config.imgBaseUrl
    }
  }
</script>
<style>
  .demo-upload-list{
    display: inline-block;
    width: 60px;
    height: 60px;
    text-align: center;
    line-height: 60px;
    border: 1px solid transparent;
    border-radius: 4px;
    overflow: hidden;
    background: #fff;
    position: relative;
    box-shadow: 0 1px 1px rgba(0,0,0,.2);
    margin-right: 4px;
  }
  .demo-upload-list img{
    width: 100%;
    height: 100%;
  }
  .demo-upload-list-cover{
    display: none;
    position: absolute;
    top: 0;
    bottom: 0;
    left: 0;
    right: 0;
    background: rgba(0,0,0,.6);
  }
  .demo-upload-list:hover .demo-upload-list-cover{
    display: block;
  }
  .demo-upload-list-cover i{
    color: #fff;
    font-size: 20px;
    cursor: pointer;
    margin: 0 2px;
  }
</style>
